...
const props = defineProps({
  ...
  static: { type: Boolean, default: false }, // 静态块：不可折叠、常开、无箭头
});

// 折叠状态：按 blockKey 全局持久化（ad-blocks），左右栏共用一套记忆
const store = JSON.parse(localStorage.getItem("ad-blocks") || "{}");
const open = ref(props.static ? true : (props.blockKey in store ? !!store[props.blockKey] : props.defaultOpen));
...
<template>
  <section class="block" :data-open="open">
    <header :class="{ static: static }" @click="!static && toggle()"><span>{{ title }}</span><i v-if="!static"></i></header>
...
.block header.static { cursor: default; }
